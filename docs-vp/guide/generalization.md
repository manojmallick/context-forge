...
description: SigMap generalizes across 21 repos, 33 languages, and multiple domains with 82.2% hit@5 in the latest saved v8.22.0 retrieval run.
head:
  - - meta
    - property: og:title
...
these 105 tasks is yes — 82.2% hit@5 with no per-repo tuning in the latest saved v8.22.0 run.
:::
...
If you want one number to carry into launch messaging, use the shared `v8.22.0` snapshot rather than an older per-page variant:
