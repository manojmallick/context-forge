...
function runPerModuleStrategy(cwd, config, fileEntries, inputTokenTotal) {
  ...
  console.warn(`[context-forge] per-module: ${moduleNames.length} modules — ${moduleNames.join(', ')}`);

  // Thin overview → primary output (always-on, tiny)
  const overviewLines = [
    '<!-- Generated by ContextForge gen-context.js v' + VERSION + ' — per-module overview -->',
  ...
  for (const mod of moduleNames) {
    ...
    totalOut += modTokens;
    console.warn(`[context-forge] per-module: wrote .github/${outName} (~${modTokens} tokens, ${budgeted.length} files)`);
  ...
  console.warn(`[context-forge] per-module: overview ~${overviewTokens} tokens (always-on), modules total ~${totalOut} tokens (on-demand)`);
...
function runHotColdStrategy(cwd, config, fileEntries, recentFiles, inputTokenTotal) {
  ...
  const hotContent = hotEntries.length > 0
    ...
    : '<!-- Generated by ContextForge — no recently changed files -->\n';
  const primaryTargets = (config.outputs || ['copilot']).filter((t) => t !== 'claude');
  ...
  const coldHeader = [
    '<!-- Generated by ContextForge gen-context.js v' + VERSION + ' — cold context (MCP only) -->',
  ...
  console.warn('[context-forge] hot-cold:');
